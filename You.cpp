...
//int C_T_t = 9, C_T_f = 1, C_W_t = 10, C_W_f = 0;
//int C_T, C_t;

//int C_T = 9, C_t = 15;
//int S_T_t = 9, S_T_f = 1, S_W_t = 10, S_W_f = 0;
//int S_T = 9, S_W = 10;
//int S_t = 15, S_w = 15;
                ...
                if (is_directory(t))
                {
                        ...
                        ++next2)
                    {
                        ...
                        if (is_regular_file(t2))
                        {
                            ...
                            if (pos != std::wstring::npos)
                            {
                                i = S.afficher_fichier(j, t2, WEBP_);
                            }
                        }
                    }
                    j++;
...
{
#if compare_path_ == 1
    //wcout << L"compare_path(" << t << L", " << t2 << L')' << endl;
    B.Ok_T(L"const int compare_path(path& t, wstring& " + t2 + L")");
    ...
    if (i == 0)
    {
        ...
        B.Ok_T(L"const int compare_path() : Ok !");
#endif
        return EXIT_SUCCESS;
    }
    for (i = 0; i < 6; i++)
    ...
    B.Ok_T(L"const int compare_path() : Erreur !!!");
#endif
    return EXIT_FAILURE;
}
